...
# 辞書を変換（多義性の扱い）
perl -I$PERL_DIR change_dic.pl --synonym=$SIM_DIR/synonym.txt --definition=$SIM_DIR/definition.txt --isa=$SIM_DIR/isa.txt --antonym=$SIM_DIR/antonym.txt --synonym_change=$SIM_C_DIR/synonym.txt --isa_change=$SIM_C_DIR/isa.txt --antonym_change=$SIM_C_DIR/antonym.txt

cp $SIM_DIR/definition.txt $SIM_C_DIR/definition.txt

perl check_duplicate_entry.pl -rnsame < $SIM_DIR/synonym_ne.txt > $SIM_C_DIR/synonym_ne.txt
