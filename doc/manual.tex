...
主なスクリプト・データを図\ref{fig:scripts}に示す。
...
while (<>) {
    ...
    if (/^EOS$/) {
        ...
        foreach my $tag ($knp_result->tag) {
            ...
            print $syngraph->tagid . ' ' . $syngraph->parent . $syngraph->dpndtype . ' ' . $syngraph->midasi;
            print  ' ' . $syngraph->feature . "\n";
...
KNP::SynGraphオブジェクトで提供されているメソッドを以下に示す。
